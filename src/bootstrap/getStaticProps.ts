...
import { resolveAncestry, linkify } from "@silverstripe/nextjs-toolkit"
...
const getStaticProps =
  (project: ProjectState): GetStaticProps =>
  async context => {
    const getQueryForType = createGetQueryForType(project)
    const api = createClient(project.projectConfig)
    const { getPropsManifest, typeAncestry, availableTemplates } =
      project.cacheManifest
    ...
    // Handle error if query does not resolve
    try {
